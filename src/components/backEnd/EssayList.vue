<template lang="html">
  <el-row>
    <el-row style="padding:10px 15px;background:#fff">
      <el-col :span="24">
        <el-button size="small" type="primary" @click="createEssay">创建随笔</el-button>
      </el-col>
    </el-row>
    <el-row>
      <el-col :span='24'>
        <el-table
          style='width:100%' align="center" :data="essayLists" v-loading="listLoading" element-loading-text="拼命加载中">
          <el-table-column type='index' width="60"></el-table-column>
          <el-table-column prop="meta.createAt" min-width="200" label="创建时间"></el-table-column>
          <el-table-column prop="classify" min-width="150" label="所属分类"></el-table-column>
          <el-table-column prop="title" min-width="180" label="随笔标题"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getLists() {
        ...
        api.getEssayListBack({
          page: this.page,
          limit: this.limit
        })
          .then(({data: {essaylist, total}}) => {
            setTimeout(() => {
              this.listLoading = false
              this.essayLists = essaylist;
              this.total = total;
            }, 500)
          })
      },
      ...
      // 跳转到编辑界面，进行新建
      createEssay() {
        this.$router.push({path: '/admin/essayCreate'});
      },
      //跳转到编辑界面，进行更新
      editArticle(id) {
        // 通过this.$route.params来获取数据
        this.$router.push({path: `/admin/essayEdit/${id}`});
      }
