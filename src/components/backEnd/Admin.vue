    ...
    <el-row class="top">
      <el-col :span='23' class="panel-top" :push="1">
        ...
        <el-col :span="20" style='font-size:26px'>
          <span><i style="color:#7ebed6">ZOMBIE</i></span>
        </el-col>
        ...
        <el-menu router :default-active="currentPath">
          <template v-for="(item,index) in $router.options.routes" v-if='!item.hidden'>
            <el-submenu :index="index+''">
              <template slot="title"><i :class="item.iconCls"></i>{{item.name||"管理面板"}}</template>
              <el-menu-item v-for="child in item.children" :key="item.name" :index="item.path+'/'+child.path"
                            v-if="!child.hidden">
                {{child.name}}
              </el-menu-item>
            </el-submenu>
